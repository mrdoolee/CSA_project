...
export const CraDataModal: React.FC<CraDataModalProps> = ({
  ...
  // Sample CSV generator for Past Seating Data
  const handleDownloadPastSeatingSample = () => {
    const sampleData = [
      ['학번', '이름', '자리배치(좌석/짝명)', '비고'],
      ['10101', '학생01', '1행 2열 (짝: 학생02)', '1분기 배치'],
      ['10102', '학생02', '2행 3열 (짝: 학생03)', '2분기 배치'],
      ['10103', '학생03', '3행 1열', ''],
      ['10104', '학생04', '2행 3열 (짝: 학생05)', ''],
      ['10105', '학생05', '4행 2열', ''],
    ];

    const ws = XLSX.utils.aoa_to_sheet(sampleData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, '자리배치양식샘플');
    XLSX.writeFile(wb, '자리배치_양식_샘플.xlsx');
  };
        ...
        try {
          ...
          const updatedStudents = students.map((st) => {
            ...
            if (assignedDeskId && desksList.length > 0) {
              ...
              if (myDesk) {
                const neighborDeskIds = desksList
                  .filter(
                    (otherDesk) =>
                      otherDesk.id !== myDesk.id &&
                      !otherDesk.disabled &&
                      ((myDesk.podId && otherDesk.podId && myDesk.podId === otherDesk.podId) ||
                        (Math.abs(myDesk.row - otherDesk.row) <= 1 && Math.abs(myDesk.col - otherDesk.col) <= 1))
                  )
                  .map((d) => d.id);

                const neighborNames = neighborDeskIds
                  .map((dId) => deskToStudentMap.get(dId))
                  .filter(Boolean)
                  .map((stId) => getStudentName(stId!));

                const seatDesc = `${myDesk.row + 1}행 ${myDesk.col + 1}열${
                  neighborNames.length > 0 ? ` (짝: ${neighborNames.join(', ')})` : ''
                }`;
  ...
  const handleDownloadBasicSample = () => {
    const sampleData = [
      ['학번', '이름', '성별', '이전자리배치', '비고(특이사항)'],
      ['10101', '학생01', '남', '1행 2열 (짝: 학생02)', '시력 보호 (앞줄)'],
      ['10102', '학생02', '여', '2행 3열', '학급 부반장'],
      ['10103', '학생03', '남', '3행 1열', '체구 작음'],
      ['10104', '학생04', '남', '1행 1열', '비고없음'],
      ['10105', '학생05', '여', '4행 2열', '청력 고려'],
    ];
  ...
  const handleDownloadCraSample = () => {
    const sampleData = [
      ...
      ['학생01', '12', '36', '0.045', '모둠_1', '4', '12', '0.015', '모둠_1', '5', '15', '0.08', '모둠_1', '3', '9', '0.02', '모둠_1', '2', '4', '0.01', '모둠_1'],
      ['학생02', '18', '58', '0.062', '모둠_1', '5', '16', '0.022', '모둠_1', '8', '26', '0.12', '모둠_1', '4', '12', '0.035', '모둠_2', '1', '2', '0.015', '모둠_1'],
      ['학생03', '15', '48', '0.055', '모둠_2', '6', '18', '0.04', '모둠_2', '4', '12', '0.03', '모둠_2', '8', '24', '0.11', '모둠_1', '3', '6', '0.04', '모둠_2'],
      ['학생04', '8', '24', '0.02', '모둠_2', '2', '6', '0.01', '모둠_2', '3', '9', '0.02', '모둠_2', '1', '3', '0.005', '모둠_3', '2', '4', '0.015', '모둠_2'],
      ['학생05', '14', '42', '0.048', '모둠_3', '5', '15', '0.025', '모둠_3', '6', '18', '0.07', '모둠_3', '2', '6', '0.01', '모둠_3', '1', '2', '0.005', '모둠_3'],
    ];
  ...
  const handleBasicFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        const headerRow = jsonData[0] as string[];
        let numIdx = -1, nameIdx = -1, genderIdx = -1, pastIdx = -1, notesIdx = -1;
        ...
        const mergedStudents: CraStudent[] = newBasicList.map((basic, idx) => {
          ...
          return {
            ...
            expansionNominated: existing?.expansionNominated || 0,
            expansionWeighted: existing?.expansionWeighted || 0,
            expansionMediator: existing?.expansionMediator || 0,
            expansionGroup: existing?.expansionGroup || '모둠_1',
          };
        });
  ...
  const handleCraFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        // Domain 4: Peer Relationship Expansion
        const expNom = findCol('4_교우관계', '지목횟수') !== -1 ? findCol('4_교우관계', '지목횟수') : headerRow.findIndex((h) => h.includes('확장') && h.includes('지명'));
        const expWgt = findCol('4_교우관계', '가중점수');
        ...
        for (let i = 1; i < jsonData.length; i++) {
          ...
          const craDataPartial = {
            ...
            expansionNominated: getNum(expNom),
            expansionWeighted: expWgt !== -1 ? getNum(expWgt) : getNum(expNom) * 2,
            expansionMediator: getNum(expMed),
            expansionGroup: getStr(expGrp, '모둠_1'),
          };
  ...
  const filteredStudents = students.filter((s) => {
    ...
    if (selectedGroup === 'expansion_desire')
      return matchesSearch && (s.expansionWeighted >= 2 || s.expansionNominated >= 1);
              ...
              <div>
                ...
                <div className="mt-4 p-4 bg-slate-50 rounded-2xl border border-slate-200 text-xs text-slate-600 space-y-2">
                  ...
                  <ul className="list-disc list-inside space-y-1 text-slate-500 text-[11px] leading-relaxed">
                    ...
                    <li>
                      <strong className="text-slate-700">선택 항목:</strong> 이전 자리배치 기록, 비고(시력, 특이사항 등)
                    </li>
                  </ul>
                </div>
              </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            <div className="p-4 bg-emerald-50/60 rounded-2xl border border-emerald-100 shadow-xs">
              ...
              <div className="text-2xl font-black text-emerald-900 mt-2">
                {students.filter((s) => s.expansionNominated > 0).length}{' '}
          ...
          <div className="bg-white rounded-3xl p-6 w-full max-w-md shadow-2xl space-y-4">
            ...
            <div>
              ...
              <input
                ...
                placeholder="예: 시력 보호 (앞줄 배치), 청력 고려 등"
                className="w-full px-3 py-2 text-xs font-semibold bg-slate-50 border border-slate-300 rounded-xl"
              />
            </div>
