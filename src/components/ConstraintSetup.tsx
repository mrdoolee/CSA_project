...
import { hasCraData } from '../utils/craAlgorithm';
...
export const ConstraintSetup: React.FC<ConstraintSetupProps> = ({
  ...
  const handleWeightChange = (key: 'intimacyWeight' | 'expansionWeight' | 'influenceWeight', val: number) => {
        ...
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 space-y-4">
          ...
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 space-y-4">
            ...
            <div className="p-3.5 bg-indigo-50/80 border border-indigo-200 rounded-2xl space-y-1.5 text-xs text-indigo-950">
              <div className="flex items-start gap-2">
                ...
                <div>
                  <span className="font-extrabold text-indigo-950">📐 3×3 영역 적용 & 필수 조건 준수 원칙:</span>
                  <p className="text-[11px] text-indigo-800 mt-0.5 leading-relaxed">
                    한 학생을 기준으로 <strong className="font-extrabold text-indigo-950">양옆, 앞뒤, 대각선 4방향(최대 8개 주변 좌석)</strong>을 '같이/따로 앉기'의 근처 범위로 정확히 판정합니다.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 space-y-4">
            ...
            <div className="space-y-2.5">
              <button
                type="button"
                onClick={() => setConstraints({ ...constraints, genderRule: 'mixed_pairs' })}
                className={`w-full p-3.5 rounded-2xl border text-left transition-all flex items-center justify-between cursor-pointer ${
                  constraints.genderRule === 'mixed_pairs'
                    ? 'bg-indigo-50 border-indigo-500 text-indigo-900 ring-2 ring-indigo-500/20'
                    : 'bg-slate-50 border-slate-200 hover:bg-slate-100 text-slate-700'
                }`}
              >
                <div>
                  <div className="font-bold text-xs">👫 남여 짝궁 고정 배치</div>
                  <div className="text-[11px] text-slate-500 mt-0.5">
                    2인 짝궁 구조 시 남학생과 여학생이 짝이 되도록 설정
                  </div>
                </div>
                <input
                  type="radio"
                  checked={constraints.genderRule === 'mixed_pairs'}
                  onChange={() => {}}
              ...
              >
                <div>
                  <div className="font-bold text-xs font-black text-indigo-700">⚖️ 남여 균등 배치 (남여 교대/혼합)</div>
                  <div className="text-[11px] text-slate-500 mt-0.5">
                    남학생과 여학생을 교실 전체 및 모둠에 교대/균등하게 안배
                  </div>
                </div>
                <input
                  type="radio"
                ...
                <div className={`space-y-3 ${!craAvailable ? 'opacity-50 pointer-events-none select-none' : ''}`}>
                  ...
                  >
                    <div className="pr-4">
                      ...
                      <p className="text-[11px] text-slate-500 mt-1 leading-relaxed">
                        CRA 정서적 친밀감 소집단에 속한 친한 친구들끼리 3×3 동일 이웃 공간에 3명 이상 몰려 집단화되지 않도록 학급 전체에 균등하게 흩어지도록 가중치를 부여합니다.
                      </p>
                    </div>
                  ...
                  {/* Prioritize Peer Expansion Toggle Card */}
                  <button
                    type="button"
                    disabled={!craAvailable}
                    onClick={() =>
                      setConstraints({
                        ...constraints,
                        prioritizeExpansion: !constraints.prioritizeExpansion,
                      })
                    }
                    className={`w-full p-3.5 rounded-2xl border text-left transition-all flex items-center justify-between cursor-pointer ${
                      constraints.prioritizeExpansion
                        ? 'bg-emerald-50 border-emerald-400 ring-2 ring-emerald-400/30 text-emerald-950'
                        : 'bg-slate-50 border-slate-200 hover:bg-slate-100 text-slate-700'
                    }`}
                  >
                    <div className="pr-4">
                      <div className="flex items-center gap-2">
                        <span className="font-black text-xs text-emerald-900">
                          🌱 교우관계 확장 최우선 (맞지목 친구 & 새 친구 연결)
                        </span>
                        {constraints.prioritizeExpansion && (
                          <span className="px-2 py-0.5 text-[10px] font-extrabold bg-emerald-600 text-white rounded-md">
                            ON
                          </span>
                        )}
                      </div>
                      <p className="text-[11px] text-slate-500 mt-1 leading-relaxed">
                        CRA 데이터의 '4_교우관계확장' 지표 중 서로를 지목한 <strong>'맞지목 친구'</strong> 및 새로운 관계 교류 희망 학생들을 3×3 근처 자리에 최우선으로 배치하도록 소프트 가중치를 극대화합니다.
                      </p>
                    </div>

                    <div
                      className={`w-11 h-6 rounded-full transition-colors flex items-center p-1 ${
                        constraints.prioritizeExpansion ? 'bg-emerald-600 justify-end' : 'bg-slate-300 justify-start'
                      }`}
                    >
                      <div className="w-4 h-4 bg-white rounded-full shadow-md" />
                    </div>
                  ...
                  {(() => {
                    ...
                    const expansionW = weights.expansionWeight || 0;
                    const influenceW = weights.influenceWeight || 0;
                    const totalW = intimacyW + expansionW + influenceW;

                    const intimacyPercent = totalW > 0 ? ((intimacyW / totalW) * 100).toFixed(1) : '33.3';
                    const expansionPercent = totalW > 0 ? ((expansionW / totalW) * 100).toFixed(1) : '33.3';
                        ...
                        <div className="space-y-1 bg-white p-2.5 rounded-xl border border-indigo-100">
                          ...
                          <div className="h-3.5 w-full bg-slate-100 rounded-lg overflow-hidden flex shadow-inner">
                            ...
                            <div
                              ...
                              title={`교우관계 확장: ${expansionPercent}%`}
                            >
                              {Number(expansionPercent) >= 12 && `${expansionPercent}%`}
                            </div>
                            <div
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                          ...
                          <div className="bg-white p-3 rounded-xl border border-emerald-100 shadow-xs space-y-2">
                            ...
                            <input
                              ...
                              max="100"
                              step="5"
                              value={weights.expansionWeight}
                              onChange={(e) => handleWeightChange('expansionWeight', Number(e.target.value))}
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h2 className="text-lg font-black text-slate-900">
                    📐 CRA 자리배치 알고리즘 산출 원리 & 가중치 계산 안내
                  </h2>
                  <p className="text-xs text-slate-500 mt-0.5">
                    교실 서시오그램(Socio-gram) 관계망 수치가 좌석에 반영되는 수학적 메커니즘
                  </p>
                </div>
              </div>
            ...
            <div className="space-y-4 text-xs text-slate-700 leading-relaxed max-h-[60vh] overflow-y-auto pr-2">
              <div className="p-4 bg-indigo-50/80 border border-indigo-200 rounded-2xl space-y-2">
                <h3 className="font-extrabold text-sm text-indigo-950 flex items-center gap-1.5">
                  ...
                  1. 3×3 격자 이웃(Neighborhood) 평가 범위
                </h3>
                <p>
                  자리배치 알고리즘은 타깃 학생 한 명을 기준으로 <strong className="text-indigo-900 font-extrabold">양옆(좌/우), 앞뒤(상/하), 그리고 4방향 대각선</strong>까지 포함하여 주변 8개 좌석을 하나의 3×3 상호작용 이웃 범주로 지정합니다.
                </p>
              </div>

              <div className="space-y-3">
                <h3 className="font-extrabold text-sm text-slate-900 border-b border-slate-200 pb-1">
                  2. 3대 CRA 교우관계 지표(S값) 세부 수치 산출 공식
                </h3>
                ...
                <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-1">
                  ...
                  <p className="text-slate-600 text-[11px] leading-relaxed">
                    동일한 정서적 친밀 소모임(intimacyGroup) 학생끼리 3×3 주변 자리에 이웃하게 배치될 때마다 감점 패널티를 부여합니다. 단짝끼리 한 구역에 무리 짓지 않고 반 전체에 고르게 분산될수록 100점에 수렴합니다.
                  </p>
                </div>

                <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-1">
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-emerald-900">🌱 교우관계 확장 지수 (S_expansion)</span>
                    <span className="text-[11px] font-extrabold text-emerald-600">맞지목 친구 최우선 보상</span>
                  </div>
                  <p className="text-slate-600 text-[11px] leading-relaxed">
                    CRA '4_교우관계확장' 지표 중 서로를 희망 상대로 지목한 <strong>'맞지목 친구'</strong> 또는 교우관계 확장을 원하는 학생이 3×3 이웃에 새로운 소모임 학생과 연결되었을 때 최대 1.0~0.7점의 가산점 보상(Reward)을 부여하여 계산합니다.
                  </p>
                </div>
                ...
                <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-1">
                  ...
                  <p className="text-slate-600 text-[11px] leading-relaxed">
                    학급 내 지명도/리더십 수치가 높은 핵심 영향력 학생들(Weighted ≥ 15 또는 Nominated ≥ 5)이 특정 모둠/구역에 겹치지 않고 반 전체 모둠 리더로 고르게 배치될 때 높은 점수를 산출합니다.
                  </p>
                </div>
              </div>
              ...
              <div className="p-4 bg-amber-50/80 border border-amber-200 rounded-2xl space-y-2">
                ...
                <p className="font-mono bg-white p-2.5 rounded-xl border border-amber-200 text-[11px] text-amber-900 font-bold leading-relaxed">
                  CRA Weighted Score = (S_intimacy × W_intimacy + S_expansion × W_expansion + S_influence × W_influence) / (W_intimacy + W_expansion + W_influence)
                  <br />
                  Final Score = (필수 제약조건 준수율 × 0.3) + (CRA Weighted Score × 0.7)
                </p>
                <div className="text-slate-600 text-[11px] space-y-1">
                  <p>• <strong>하드 제약조건 (Hard Constraints)</strong>: [시각적 고정석], [같이/따로 앉기] 등은 100% 필수 준수해야 하며 위반 시 절대 선택되지 않습니다.</p>
                  <p>• <strong>소프트 가중치 (Soft Weights)</strong>: 교사가 설정한 슬라이더 비중($W$)에 따라 3대 교우관계 지표의 상대적 중요도가 종합 점수에 실시간 반영됩니다.</p>
                </div>
              </div>
            </div>
