      ...
      <div className="bg-white rounded-3xl p-6 w-full max-w-5xl shadow-2xl space-y-6 relative max-h-[92vh] flex flex-col printable-area">
        ...
        <div className="flex-1 overflow-y-auto space-y-6 pr-1 print:overflow-visible print:pr-0">
          ...
          <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl grid grid-cols-2 sm:grid-cols-4 gap-3 text-center no-print">
            ...
            <div className="bg-white p-2.5 rounded-xl border border-slate-200 shadow-xs">
              <div className="text-[11px] font-bold text-slate-400">교우관계 확장</div>
              <div className="text-lg font-black text-emerald-600">
                {result.metrics?.expansionScore || 90}점
              </div>
            </div>
            <div className="bg-white p-2.5 rounded-xl border border-slate-200 shadow-xs">
                ...
                return (
                  <React.Fragment key={`restored_r_${r}`}>
                    <div className="flex items-center gap-2 sm:gap-3">
                      <span className="text-xs font-bold text-slate-500 w-7 text-right shrink-0 print:text-slate-800">
                        {r + 1}행
                      </span>
                      <div className="flex items-center gap-2 sm:gap-3">
        ...
        <div className="pt-4 border-t border-slate-200 flex items-center justify-between gap-3 shrink-0 no-print">
          <p className="text-xs text-slate-500">
            복원된 자리배치표입니다. 상단 [자리배치표 인쇄] 버튼으로 제목과 설명글이 포함된 자리배치표를 즉시 인쇄할 수 있습니다.
          </p>
