...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const [isClassMenuOpen, setIsClassMenuOpen] = useState(true);

  const teacherItems = [
    {
      id: 'cra' as const,
      label: '1. 학생 & CRA 데이터 관리',
      subLabel: '학번·성별·비고·이전자리',
      icon: Users,
    },
    {
      id: 'layout' as const,
  ...
  const classItems = [
    {
      id: 'raffleAndVote' as const,
      label: '🎉 학급 자리배치 추첨',
      subLabel: '학생 전용 슬롯 라이브 추첨',
      icon: Trophy,
    },
  ];
          ...
          <div className="space-y-1">
            {!isCollapsed && (
              <button
                ...
                className="w-full pt-1 pb-1 px-2 text-[11px] font-black uppercase tracking-wider text-amber-400/90 flex items-center justify-between hover:text-amber-300 cursor-pointer"
              >
                <span>학급 이벤트 메뉴</span>
                {isClassMenuOpen ? <ChevronUp className="w-3.5 h-3.5" /> : <ChevronDown className="w-3.5 h-3.5" />}
              </button>
            )}

            {(isClassMenuOpen || isCollapsed) &&
              classItems.map((item) => {
                const Icon = item.icon;
                const isActive = activeTab === item.id;
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 p-3 rounded-2xl text-left transition-all cursor-pointer ${
                      isActive
                        ? 'bg-amber-500/20 text-amber-300 border border-amber-500/30 font-bold shadow-md shadow-amber-950/50'
