...
export const ResultView: React.FC<ResultViewProps> = ({
  ...
  const [printTitle, setPrintTitle] = useState<string>('2학년 1반 학급 자리배치표');
  ...
  const handleExportToExcel = (cand: SeatingResult) => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, '자리배치결과');
  ...
  const handleRefreshCandidate4 = (e?: React.MouseEvent) => {
    ...
    const updatedCand: SeatingResult = {
      ...cand4,
      id: 'opt_4',
      title: '후보 4: 완전 무작위 배치 (CRA 무시)',
      description: 'CRA 관계망 수치를 전혀 반영하지 않고, 지정한 기본 제약조건만 준수하며 100% 무작위로 배치합니다.',
    ...
    <div className="space-y-6">
            ...
            <div className="flex items-center justify-between border-b border-slate-200 pb-4 no-print">
              <div>
                <h2 className="text-lg sm:text-xl font-black text-slate-900">
                  🖨️ 학급 자리배치 인쇄용 도면 ({currentCandidate.title})
                </h2>
                <p className="text-xs text-slate-500 mt-0.5">
                  '모둠', '행-열', '성별' 등의 정보는 제외하고 **학번**과 **이름**만 깔끔하게 출력합니다.
                </p>
              </div>
                          ...
                          {Array.from({ length: dimensions.cols }).map((_, c) => {
                            ...
                            return (
                              <React.Fragment key={`print_d_${r}_${c}`}>
                                {desk && !desk.disabled ? (
                                  <div className="w-24 sm:w-28 h-16 sm:h-20 border-2 border-slate-900 rounded-xl bg-white p-1 flex flex-col justify-center items-center text-center shadow-2xs shrink-0">
                                    {assignments[desk.id] ? (
                                      <>
      ...
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-slate-100">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">
              CRA 관계망 알고리즘으로 생성된 자리배치 후보 1~3안, 무작위 또는 수동 지정 자리배치 후보 4안을 비교·수정하고, [학생 자리배치 추첨]에 포함할 후보를 선택하세요.
            </p>
          </div>
            ...
            return (
              ...
              >
                <div>
                  ...
                  {/* Student Raffle Inclusion Checkbox Toggle */}
                  <button
                    type="button"
                    onClick={(e) => {
                      e.stopPropagation();
                      onToggleRaffleCandidate(cand.id);
                    }}
                    className={`w-full mt-2 py-1 px-2 rounded-xl text-[10px] font-black flex items-center justify-center gap-1 transition-all cursor-pointer border ${
                      isRaffleIncluded
                        ? 'bg-purple-100 text-purple-900 border-purple-300 ring-1 ring-purple-400/30'
                        : 'bg-white text-slate-500 border-slate-200 hover:text-slate-700 hover:bg-slate-50'
                    }`}
                    title="학생 자리배치 추첨 라이브 화면에 이 후보를 포함할지 선택합니다."
                  >
                    <input
                      type="checkbox"
                      checked={isRaffleIncluded}
                      readOnly
                      className="accent-purple-600 cursor-pointer pointer-events-none w-3 h-3"
                    />
                    <span>🎯 추첨 연출 포함</span>
                  </button>
                ...
                <div className="mt-3 pt-2.5 border-t border-slate-200/80 space-y-2">
                  <div className="grid grid-cols-2 gap-1 text-[10px] font-bold">
                    <div>
                      <span className="text-slate-400">교우 확장:</span>{' '}
                      <span className="text-emerald-700 font-extrabold">{cand.metrics.expansionScore}점</span>
                    </div>
                    <div>
        ...
        <div className="p-4 bg-slate-900 rounded-3xl border border-slate-800 text-white flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-center flex-1 w-full">
            ...
            <div className="p-2 border-r border-slate-800">
              <div className="text-[11px] text-emerald-400 font-semibold">🌱 교우관계 확장 지수</div>
              <div className="text-xl font-bold text-slate-100 mt-1">
                {currentCandidate.metrics.expansionScore}점
              </div>
            </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-slate-800 text-xs">
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              title="A4 양식 학급 자리배치표 전용 인쇄 창 열기"
            >
              <Printer className="w-3.5 h-3.5 text-white" />
              <span>🖨️ 자리배치표 인쇄</span>
            </button>
          </div>
            ...
            return (
              <React.Fragment key={`res_r_frag_${r}`}>
                <div className="flex items-center gap-3">
                  <span className="text-xs font-bold text-slate-500 w-8 text-right">{r + 1}행</span>
                  <div className="flex items-center gap-3">
                    {Array.from({ length: dimensions.cols }).map((_, c) => {
                      const hasColAisle = dimensions.colAisles?.includes(c);
                      const desk = desks.find((d) => d.row === r && d.col === c);
                      ...
                      return (
                        <React.Fragment key={desk.id}>
                          <button
                            ...
                            className={`w-28 h-20 rounded-2xl border-2 p-2 flex flex-col justify-between transition-all relative text-left cursor-pointer ${
                              isSelectedForSwap
                                ? 'bg-amber-950/90 border-amber-400 ring-4 ring-amber-400/40 scale-105 z-10'
                                : isGroupHighlighted
                                ? 'bg-indigo-900/90 border-indigo-400 ring-2 ring-indigo-400/50 scale-102'
                                : 'bg-slate-900/80 border-slate-700/80 hover:border-indigo-500 hover:scale-102'
                            }`}
                          >
                            ...
                            {student && (
                              <div className="flex items-center justify-between text-[9px] font-medium text-slate-400">
                                ...
                                <div className="flex items-center gap-0.5 text-[11px]">
                                  ...
                                  {student.notes && (student.notes.includes('시력') || student.notes.includes('앞자리') || student.notes.includes('청력')) && (
          ...
          <div className="bg-white p-6 md:p-8 rounded-3xl border border-slate-200 shadow-2xl max-w-2xl w-full space-y-6">
            ...
            <div className="space-y-4 text-xs text-slate-700 leading-relaxed max-h-[60vh] overflow-y-auto pr-2">
              <div className="p-4 bg-indigo-50/90 border border-indigo-200 rounded-2xl flex items-center justify-between">
                <div>
                  ...
                  <p className="text-[11px] text-indigo-700 mt-0.5">
                    제약조건 준수율(30%) + 교사 가중치가 반영된 CRA 점수(70%)
                  </p>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div className="p-3.5 bg-slate-50 border border-slate-200 rounded-xl space-y-1">
                  ...
                  <p className="text-[11px] text-slate-500 leading-tight">
                    단짝 소집단 동시 밀집 패널티 차감 후 산출된 교실 분산 지수입니다.
                  </p>
                </div>

                <div className="p-3.5 bg-slate-50 border border-slate-200 rounded-xl space-y-1">
                  <div className="flex items-center justify-between">
                    <span className="font-extrabold text-emerald-900">🌱 교우관계 확장 지수</span>
                    <span className="font-black text-emerald-600">{currentCandidate.metrics.expansionScore}점</span>
                  </div>
                  <p className="text-[11px] text-slate-500 leading-tight">
                    새 친구 사귀기 지목 학생이 3×3 이웃에 연결된 비율입니다.
                  </p>
                </div>
                ...
                <div className="p-3.5 bg-slate-50 border border-slate-200 rounded-xl space-y-1">
                  ...
                  <p className="text-[11px] text-slate-500 leading-tight">
                    학급 리더십 수치 학생 간 3×3 중복 충돌 회피 점수입니다.
                  </p>
                </div>
                ...
                <div className="p-3.5 bg-slate-50 border border-slate-200 rounded-xl space-y-1">
                  ...
                  <p className="text-[11px] text-slate-500 leading-tight">
                    고정석, 같이/따로 앉기, 성별 및 과거 짝 회피 규칙 성립율입니다.
                  </p>
                </div>
              </div>
              ...
              <div className="p-3.5 bg-slate-100 rounded-xl border border-slate-200 text-slate-600 text-[11px] space-y-1">
                ...
                <p>
                  3단계 [CRA 알고리즘 가중치 설정]에서 슬라이더를 통해 각 지표의 상대적 중요도 비중(%)을 조정하면 최종 종합 점수 산출 방식에 실시간 적용됩니다.
                </p>
              </div>
            </div>
